...
public class TimeUtils {
	...
	public static String getTimeDiff(long millis){
		long day;
		long hour;
		long min;
		long sec;
		...
		return day+"天"+(hour>9?hour:("0"+hour))+"小时"+(min>9?min:("0"+min))+"分"+(sec>9?sec:("0"+sec))+"秒";
	}


	public static String getTimeDiffString(long millis){
		long day;
		long hour;
		long min;
		long sec;
		...
		min=((l/(60*1000))-day*24*60-hour*60);
		sec = ((l/(1000))-day*24*60*60-hour*60*60-min*60);
		return day+","+(hour>9?hour:("0"+hour))+","+(min>9?min:("0"+min))+","+(sec>9?sec:("0"+sec))+",";
	}


	/**
	 * 获取时间差
	 * @param startTimeStr yyyy-MM-dd HH:mm:ss
	 * @param endTimeStr yyyy-MM-dd HH:mm:ss
     * @return 相差的毫秒
     */
	public static long getTimeDifference(String startTimeStr,String endTimeStr){
		...
		Date date2 = null;
		try {
			date1 = df.parse(startTimeStr);
			date2 = df.parse(endTimeStr);
