package com.zhejiang.haoxiadan.third.jiguang.chat.adapter;
...
import com.zhejiang.haoxiadan.third.jiguang.chat.utils.TimeFormat;
import com.zhejiang.haoxiadan.third.jiguang.chat.utils.ViewHolder;
import com.zhejiang.haoxiadan.third.jiguang.chat.view.ConversationListView;
import com.zhejiang.haoxiadan.third.jiguang.chat.view.SwipeLayoutConv;
...
public class ConversationListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        if (convItem.getType().equals(ConversationType.single)) {
            ...
            if (feedBack != null && feedBack.getUserName().equals("feedback_Android")) {
        ...
        if (TextUtils.isEmpty(draft)) {
            ...
            if (lastMsg != null) {
                String storeId = lastMsg.getContent().getStringExtra("storeId");
                String buyName = lastMsg.getContent().getStringExtra("buyName");
                String buyImg = lastMsg.getContent().getStringExtra("buyImg");
                String buyId = lastMsg.getContent().getStringExtra("buyId");
                String storeName = lastMsg.getContent().getStringExtra("storeName");
                String storeLogo = lastMsg.getContent().getStringExtra("storeLogo");
                String customerPhone = lastMsg.getContent().getStringExtra("customerPhone");
                String storePhone = lastMsg.getContent().getStringExtra("storePhone");
                String userType = lastMsg.getContent().getStringExtra("userType");//发送消息的人类型
                String userType2 = (String) SharedPreferencesUtil.get(mContext,Constants.userRole,"");//当前用户类型

                switch (userType2){
                    case Constants.USER_BUY:
                        ImageLoaderUtil.displayImageAndDefaultImg(storeLogo,headIcon, R.mipmap.ic_launcher);
                        convName.setText(storeName);
                        break;
                    case Constants.USER_SELL:
                        ImageLoaderUtil.displayImageAndDefaultImg(buyImg,headIcon, R.mipmap.ic_launcher);
                        convName.setText(buyName);
                        break;
                ...
                if (mAtAll.get(position) && Constants.isAtall.get(gid) != null && Constants.isAtall.get(gid)) {
                ...
                } else if (mArray.get(position) && Constants.isAtMe.get(gid) != null && Constants.isAtMe.get(gid)) {
                    //有人@我 文字提示
                    SpannableStringBuilder builder = new SpannableStringBuilder(contentStr);
                    builder.setSpan(new ForegroundColorSpan(Color.RED), 0, 6, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    content.setText(builder);
                    ...
                    if (lastMsg.getTargetType() == ConversationType.group && !contentStr.equals("[群成员变动]")) {
                        ...
                        if (Constants.isAtall.get(gid) != null && Constants.isAtall.get(gid)) {
                            content.setText("[@所有人] " + fromName + ": " + contentStr);
                        } else if (Constants.isAtMe.get(gid) != null && Constants.isAtMe.get(gid)) {
                        ...
                        if (Constants.isAtall.get(gid) != null && Constants.isAtall.get(gid)) {
                            ...
                            try {
                                String goodsId = lastMsg.getContent().getStringExtra("goodsId");
                                if(null != goodsId && !"".equals(goodsId)){
                                    try {
                                        ...
                                        content.setText(mContext.getString(R.string.type_picture) + goodsName);
