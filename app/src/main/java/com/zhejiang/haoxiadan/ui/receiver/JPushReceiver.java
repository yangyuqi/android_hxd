package com.zhejiang.haoxiadan.ui.receiver;
...
public class JPushReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			Log.d(TAG, "[JPushReceiver] 接收到推送下来的内容: " + alert);


			String data = bundle.getString(JPushInterface.EXTRA_EXTRA);
			GlobalDataUtil.putObject(context,Constants.PUSH_RECEIVER_TIP_LOGIN_DATA,data);
			EventBus.getDefault().post(Event.TIP_GOTO_LOGIN);
			...
			if(data!=null && !"".equals(data)){
				try {
					...
					PushData pushData = mGson.fromJson(data,PushData.class);
					if(pushData==null){
						return;
					}
					...
					if(pushData.getType()!=null){
						switch (pushData.getType()){
							...
							case "order":
								...
								orderIntent.putExtra("orderId",pushData.getData_id());
								intents[1] = orderIntent;
								context.startActivities(intents);
								break;
						}
					}
					Log.d(TAG, "[JPushReceiver] 点击打开了的内容: " + data);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void processCustomMessage(Context context, Bundle bundle) {
		...
		//自定义信息： 获取
		Intent i = new Intent();
		i.setAction(JPushInterface.ACTION_NOTIFICATION_OPENED);
		i.putExtras(bundle);
		PendingIntent pi =  PendingIntent.getBroadcast(context,1000,i,PendingIntent.FLAG_UPDATE_CURRENT);
		...
		notifyBuilder.setSmallIcon(R.mipmap.icon_logo);
		notification = notifyBuilder.build();
		notification.defaults |= Notification.DEFAULT_SOUND;
		notification.defaults |= Notification.DEFAULT_VIBRATE;
		notifyManager.notify(notifactionId, notification);
