...
import com.zhejiang.haoxiadan.third.jiguang.chat.pickerimage.utils.AttachmentStore;
import com.zhejiang.haoxiadan.third.jiguang.chat.pickerimage.utils.StorageUtil;
import com.zhejiang.haoxiadan.third.jiguang.chat.utils.BitmapLoader;
import com.zhejiang.haoxiadan.third.jiguang.chat.utils.DialogCreator;
import com.zhejiang.haoxiadan.third.jiguang.chat.utils.NativeImageLoader;
import com.zhejiang.haoxiadan.third.jiguang.chat.view.ImgBrowserViewPager;
import com.zhejiang.haoxiadan.third.jiguang.chat.view.PhotoView;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(Constants.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(Constants.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(Constants.TARGET_ID);
            if (targetId != null) {
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(Constants.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(Constants.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(Constants.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(Constants.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        ...
                        activity.setResult(Constants.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
